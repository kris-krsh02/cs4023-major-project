...
class TurtlebotTouringEnv(gym.Env):
        ...
        while collision is None:
            try:
                collision = rospy.wait_for_message(
                    "/mobile_base/events/bumper", BumperEvent, timeout=5
                )
            except:
                rospy.loginfo("Bumper has no data, retrying...")

        # Distance to goal and Heading angle
        pose = None
        while pose is None:
            try:
                pose = rospy.wait_for_message("/odom", Odometry, timeout=5)
            except:
                rospy.loginfo("Odometer has no data, retrying...")

        scan_ranges, scan_angles = self.process_scan_ranges(scan_ranges)
        distance = self.calculate_distance(pose.pose.position, self.curr_goal)
        heading_angle = self.calculate_heading_angle(pose)

        return (scan_ranges, collision, distance, heading_angle, scan_angles)
    ...
    def process_scan_ranges(self, scan_ranges):
        ...
        return filtered_scan_ranges, scan_angles
    ...
    def calculate_reward(self, data: tuple):
        ...
        if data[1]:  # collision detection
            ...
            obstacle_reward = self.calculate_weighted_obstacle_reward(data[0], data[5])
